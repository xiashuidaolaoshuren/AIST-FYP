...
This module is inspired by the **SelfCheckGPT** framework [9], which posits that hallucinations are often associated with high model uncertainty. The core idea is that if an LLM "knows" a fact, it will generate it consistently with high confidence (low entropy). Conversely, if the model is hallucinating, it is essentially "guessing," resulting in a flatter probability distribution over the vocabulary and thus higher entropy.
