...
Through our rigorous evaluation via the RAGTruth and CiteBench frameworks, we demonstrate that a zero-shot, multi-signal architecture can successfully function as a safety-first hallucination detector. The proposed *full_verifier* achieved a significantly higher hallucination detection recall (0.8196) than fine-tuned baselines like LettuceDetect, drastically reducing the rate of dangerous false negatives. Ultimately, utilizing deep semantic NLI verification as the core signal for downstream mitigation (filtering and reprompting) reliably elevated citation structural quality from "Poor" to "Good" benchmark scores (exceeding ~4.0 CE Mean Rating) and drastically preserved attribution density compared to a post-hoc detection tool. The system successfully trades minor losses in absolute precision for paramount system transparency, making it structurally favorable for deployed safety-critical RAG environments.

Future iterations of this project will explore substituting zero-shot components with task-specific supervised verifiers (via the CiteEval benchmark [14]) and designing an optimized end-user UI for reviewing generated citations and claim verdicts interactively. Furthermore, dynamically adjusting weighting policies for NLI conflict signals across the RuleBasedAggregator's differing RAG workloads could bridge the existing precision gap in short-form question answering applications.
